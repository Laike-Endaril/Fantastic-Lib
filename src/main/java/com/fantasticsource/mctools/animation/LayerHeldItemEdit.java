...
public class LayerHeldItemEdit extends LayerHeldItem
{
    ...
    public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        ...
        if (!itemstack.isEmpty() || !itemstack1.isEmpty())
        {
            ...
            if (handItemSwap.paths.size() > 0 && handItemSwap.getRelativePosition(millis).values[0] < 0)
            {
    ...
    private void renderHeldItem(EntityLivingBase entity, ItemStack stack, ItemCameraTransforms.TransformType transformType, EnumHandSide handSide, long millis)
    {
        ...
        if (handSide == EnumHandSide.LEFT)
        {
            ...
            if (animation.leftItem.zScalePath.paths.size() > 0)
            {
            ...
            GL11.glTranslatef(animation.leftItem.xPath.paths.size() == 0 ? 0 : (float) animation.leftItem.xPath.getRelativePosition(millis).values[0], animation.leftItem.yPath.paths.size() == 0 ? 0 : (float) animation.leftItem.yPath.getRelativePosition(millis).values[0], animation.leftItem.zPath.paths.size() == 0 ? 0 : (float) animation.leftItem.zPath.getRelativePosition(millis).values[0]);

            if (animation.leftItem.zRotPath.paths.size() > 0) GL11.glRotated(Math.toDegrees(Tools.posMod(animation.leftItem.zRotPath.getRelativePosition(millis).values[0], Math.PI * 2)), 0, 0, 1);
            if (animation.leftItem.yRotPath.paths.size() > 0) GL11.glRotated(Math.toDegrees(Tools.posMod(animation.leftItem.yRotPath.getRelativePosition(millis).values[0], Math.PI * 2)), 0, 1, 0);
            if (animation.leftItem.xRotPath.paths.size() > 0) GL11.glRotated(Math.toDegrees(Tools.posMod(animation.leftItem.xRotPath.getRelativePosition(millis).values[0], Math.PI * 2)), 1, 0, 0);
        }
        else
        {
            if (animation.rightItem.xScalePath.paths.size() > 0)
            {
                rightItemScale = new float[]{(float) animation.rightItem.xScalePath.getRelativePosition(millis).values[0], 1, 1};
            }
            if (animation.rightItem.yScalePath.paths.size() > 0)
            {
            ...
            if (animation.rightItem.zScalePath.paths.size() > 0)
            {
            ...
            GL11.glTranslatef(animation.rightItem.xPath.paths.size() == 0 ? 0 : (float) animation.rightItem.xPath.getRelativePosition(millis).values[0], animation.rightItem.yPath.paths.size() == 0 ? 0 : (float) animation.rightItem.yPath.getRelativePosition(millis).values[0], animation.rightItem.zPath.paths.size() == 0 ? 0 : (float) animation.rightItem.zPath.getRelativePosition(millis).values[0]);

            if (animation.rightItem.zRotPath.paths.size() > 0) GL11.glRotated(Math.toDegrees(Tools.posMod(animation.rightItem.zRotPath.getRelativePosition(millis).values[0], Math.PI * 2)), 0, 0, 1);
            if (animation.rightItem.yRotPath.paths.size() > 0) GL11.glRotated(Math.toDegrees(Tools.posMod(animation.rightItem.yRotPath.getRelativePosition(millis).values[0], Math.PI * 2)), 0, 1, 0);
            if (animation.rightItem.xRotPath.paths.size() > 0) GL11.glRotated(Math.toDegrees(Tools.posMod(animation.rightItem.xRotPath.getRelativePosition(millis).values[0], Math.PI * 2)), 1, 0, 0);
        }
