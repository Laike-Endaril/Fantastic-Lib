...
public class MCTools
{
    ...
    public static Entity[] withinCone(Vec3d origin, float yaw, float pitch, double range, double angle, boolean LOS, double distributedRaytraceSpacing, Entity... entitiesToCheck)
    {
        ...
        if (!LOS)
        {
            ...
            {
                double squareDist = origin.squareDistanceTo(target.getPositionVector());
                if (squareDist > squareRange) continue;

                if (angleDifDeg(origin, coneAxisEnd, target.getPositionVector().addVector(0, target.height / 2, 0)) > halfAngle) continue;
        ...
        else
        {
            for (Entity target : entitiesToCheck)
            {
                ...
                double angleDif = angleDifDeg(origin, coneAxisEnd, targetCenter);
                ...
                double subConeStep = Tools.radtodeg(TRIG_TABLE.arctan(distributedRaytraceSpacing / distance));
                ...
                for (int cone = 0; cone < subConeCount; cone++)
                {
                    ...
                    Quaternion theta0 = rotatedQuaternion(qPitchYaw, qPitchAxis, Tools.degtorad(subConeAngle));
                    ...
                    for (int thetaStepI = 0; thetaStepI < thetaStepCount; thetaStepI++)
                    {
                        ...
                        Quaternion qRotated = rotatedQuaternion(theta0, qPitchYaw, roll);
